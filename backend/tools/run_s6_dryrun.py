...
DEFAULT_WORK_DIR = _REPO_ROOT / "docs/ai-dlc/research/persona-realism-artifacts/detection-s6/dryrun"
...
def assert_manifests(pass_dir: Path) -> None:
    ...
    check(
        success["presentation_entries"] == expected_entries,
    ...
    forbidden = sorted(
        {
            ...
            *pins["bot"]["lineup"].values(),
            pins["human"]["session_id"],
            pins["bot"]["run_id"],
            pins["control"]["run_id"],
            pins["bot"]["config_hash"],
            pins["control"]["control_policy"],
            pins["control"]["control_policy_source"],
            pins["git_sha"],
        }
        - {""}
    )
    for entry in presentation["bundles"]:
        ...
        check(
            set(entry) <= {"presentation_id", "rendered_text", "sha256", "duplicate_for_slot"},
        ...
        check(
            entry["sha256"] == detection_corpus.payload_digest(pid, entry["rendered_text"]),
    ...
    check(
        all(s["class"] == "human" for s in slots),
        "a judge duplicate is not drawn from the HUMAN class (§A.3)",
    )
    phases_human = {p["phase_id"] for p in unblinding["position_phases"] if p["human_bundles"]}
    phases_bot = {p["phase_id"] for p in unblinding["position_phases"] if p["bot_bundles"]}
    check(
        phases_human == phases_bot,
...
def assert_judging(pass_dir: Path, completion: dict) -> None:
    ...
    check(
        all(j["resolved_model"] for j in launch["judges"]),
...
def assert_analysis(pass_dir: Path, analysis: dict) -> None:
    ...
    check(
        sorted(control["usable_confidences"]) == sorted(expected["confidences"]),
        "analysis control confidences do not match the stub's own output",
    )
    check(
        control["bot_label_count"] == expected["bot_label_count"],
        "analysis control bot-label count does not match the stub's own output",
    )
    check(
        analysis["batch_valid"] is True,
    ...
    deck = analysis["deck"]
    check(
        deck["counts"]["human"] == DRY_HUMAN_BUNDLES and deck["counts"]["bot"] == DRY_BOT_BUNDLES,
    ...
    check(
        not analysis["completeness"]["missing_pairs"],
        f"completeness reports missing pairs: {analysis['completeness']['missing_pairs']}",
    )
    check(
        not analysis["completeness"]["excluded_bundles"],
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--db-path",
        type=Path,
        default=detection_corpus.DEFAULT_DB_PATH,
    ...
    print(
        "S6 dry run — master/order/bootstrap seed "
        f"{DRY_MASTER_SEED}/{DRY_ORDER_SEED}/{DRY_BOOTSTRAP_SEED}"
    )
    print(
        f"  deck: {DRY_HUMAN_BUNDLES} human + {DRY_BOT_BUNDLES} bot bundles of "
    ...
    try:
        for name in PASSES:
            ...
            print(
                f"  {name}: OK — {documents['completion']['total']} judged pairs, "
    ...
    print(
        "  declared-volatile values normalized before comparison "
