...
server (localhost:7173). DB migrations auto-run on startup (alembic upgrade
...
# Dev: Vite (7173) -> FastAPI (8008) is cross-origin. In prod the SPA is
# served same-origin and this is a no-op.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:7173"],
