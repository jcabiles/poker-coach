...
def _fake_analytics(root: Path) -> Path:
    ...
    (root / "scorer" / "artifacts" / "a5_baseline_z.json").write_text(json.dumps(BASELINE_STUB))
    return root
...
def test_export_candidate_rejects_a_manifest_that_ignored_the_pins(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        dg.export_analytics, "run_export", lambda **kw: {"seed": 999, "n_hands": kw["n_hands"]}
    )
    with pytest.raises(dg.GateError) as exc:
        dg.export_candidate(tmp_path, 601, 10, {str(i): "tag" for i in range(9)})
...
def test_export_candidate_pins_buyin_spread_off(tmp_path, monkeypatch):
    ...
    captured = {}
    monkeypatch.setattr(
        dg.export_analytics, "run_export", lambda **kw: captured.update(kw) or {"run_id": "r"}
    )
    dg.export_candidate(tmp_path, 601, 10, {str(i): "tag" for i in range(9)})
    assert captured["buyin_spread"] is False
...
def test_run_check_parses_json_and_passes_self_test_flag(tmp_path, monkeypatch):
    ...
    out = dg.run_check(Path("py"), Path("ck"), Path("bl"), tmp_path, {"0": "tag"}, self_test=True)
...
def test_run_check_refuses_an_ambiguous_verdict(tmp_path, monkeypatch, stdout, code):
...
def test_run_check_omits_self_test_flag_when_judging(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(dg.subprocess, "run", lambda cmd, **kw: (seen.update(cmd=cmd), R())[1])
    dg.run_check(Path("py"), Path("ck"), Path("bl"), tmp_path, {}, self_test=False)
