...
def test_bluff_catcher_alpha_contract_untouched_at_multiple_opponents(
    ...
    ⚠️ THE EXCLUSION IS A FIXTURE SCOPE, NOT A STATEMENT ABOUT WHERE α BINDS.
    This docstring used to say the owner's 2026-08-19 ruling made α bind on
    the ACE_HIGH bucket specifically. That ruling was WITHDRAWN on 2026-08-24
    (contract amendment A9, `docs/ai-dlc/contracts/persona-realism-theory-
    contract.md`): α bounds a defender's WHOLE RANGE and nothing smaller, so
    there is no per-bucket ACE_HIGH obligation to state either way. What this
    test asserts is unaffected by any of that — it pins the one-pair catcher
    range as byte-identical across opponent counts, and says nothing about
    ace-high's own rates.
