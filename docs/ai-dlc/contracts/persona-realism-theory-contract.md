...
### A8 — the limits of the bluff-share formula, and the one lever the value side now has (S3-T3, 2026-08-22)

**Bottom line. The bluff-share formula above prices only ONE side of the betting range. The engine has levers that set how often a bot bluffs; until 2026-08-22 it had NO lever that made a bot's value betting respond to bet size or stack depth at all, so the formula could never be satisfied by moving the value side. S3-T3 added one narrow lever — a stack-to-pot damp on made-hand betting — and this amendment records both what that lever does and, more importantly, what it does NOT unblock, so that a future author does not read "the value side has a lever now" and retry a change that was already withdrawn once.**
...
**1. Before S3-T3, the value side had no size term and no stack term.** `_AGG_BASE` (`backend/app/domain/personas_postflop.py`) is indexed by strength bucket alone. Measured across all six packs, a top pair, a middle pair or an air hand bet at *identical* probability at a stack-to-pot ratio of 0.3 and of 10.0. The only stack response anywhere on the value side was the commit block's single flat step: at or below the pack's `spr_commit`, a hand of overpair rank or better has its bet and raise weights multiplied by 3.0 and its fold weight zeroed. That is a step at one threshold, not a slope, and it does not touch top pair or middle pair at all.

**2. What S3-T3 added, and its exact boundary.** `_value_spr_mult(spr, spr_commit)` multiplies the made-value BET merit — middle pair and better, on the unopened/matched-with-option BET arm only — by a factor that is exactly 1.0 at and above the pack's `spr_commit` and falls linearly to `_VALUE_SPR_FLOOR` at a stack-to-pot ratio of zero. It reads `stack_bb` and `pot_bb` and nothing else. It does not touch the bluff cell, the raising arm, the facing-a-bet branch, or the commit block's fold-zeroing. Every decision at or above `spr_commit` is bit-identical, which is a test, not a claim (`test_s3t3_every_persona_is_bit_identical_at_its_own_spr_commit`). Its magnitude is a **fit seed under §2**, not a derived constant: the multiplier is not the frequency change.

**3. The withdrawn bluff-side repricing CANNOT be offset from the value side, and this lever does not change that.** PR #199 withdrew a change that would have repriced the bluff weight at capped decisions on the size actually made rather than the size authored, at a ratio of about 0.78. To hold the betting range's composition on the formula while that happened, the value side would have to fall by roughly **25 percent** at those decisions. The total motion available at the merit layer is about **10 percent** — that figure already includes deleting the commit block's 3.0 boost outright, which is bad poker and not on offer. The gap is structural: strong buckets sit at bet probabilities near saturation, where §2's softmax law says a weight multiplier buys almost no frequency. **A future slice that wants the bluff side repriced on the realized size must re-open the target rather than reach for a bigger value damp.**

**4. The formula is a property of a betting RANGE, not of a decision.** The realized bluff share at a decision is `Σ_bluff π(cell)·P(bet | cell) ÷ Σ_all π(cell)·P(bet | cell)`, where `π` is how often the seat actually holds each cell there. That is arrival, the merit layer cannot see it, and it differs at every decision. So a merit-layer lever moves a **pooled** statistic toward the formula and can never satisfy it at one decision. Two consequences bind future work: an acceptance criterion of the form "this decision's composition equals `s/(1+2s)`" is unsatisfiable by construction and must be rejected at review; and a pooled criterion must be measured across seeds or paired, never on one seed, because the single-seed standard error on the capped-decision statistic is larger than the effect any admissible lever produces.

**5. Most of the capped-versus-uncapped composition gap is arrival, not policy, and is therefore out of the merit layer's reach.** Measured by `backend/tools/capped_composition_probe.py` over 60,000 hands on the ratified lineup, the bluff-cell share of the unopened betting range is 0.0771 where the stack caps the bet and 0.1316 where it does not. Paired at the same decisions, the whole of S3-T3's lever moves the capped figure by 0.0010. The remaining gap is that capped decisions are simply reached by stronger ranges — hands go there to get all in. **Do not read the residual gap as an unfixed policy defect; it is the composition of who arrives.**
